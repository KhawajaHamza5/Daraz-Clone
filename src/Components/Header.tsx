...
const Header: React.FC = () => {
  ...
  const { isLogined, setIsLogined, clearProducts, removeProduct } = useUserStore();
  ...
  const handleRemoveProduct = (id: number, discountPercentage: number, price: number, quantity:number) => {
    const discountPrice = ((price * discountPercentage) / 100)*quantity;
    removeProduct(id, discountPrice);
  ...
  return (
    <>
      <div className='justify-between flex items-center bg-orange-700 
        text-3xl px-2 h-[63px] md:h-[70px] md:text-[33px] md:px-3 
        lg:h-[80px] lg:text-[36px] lg:px-4
        xl:h-[85px] xl:text-[39px] xl:px-5 
        2xl:h-[90px] 2xl:text-[42px] 2xl:px-6'>
        <div>
          <img className='h-8 sd:h-9 md:h-10 lg:h-11 xl:h-12' 
      ...
      <div className={`fixed top-15 right-0 w-3/5 sm:w-1/2 md:w-2/5 lg:w-2/5
        ...
        {isLogined ?
          <div className='w-full p-2 pt-6 pb-10'>
            <div className='flex p-4 w-full'>
              <MdDeleteSweep className='text-black font-bold text-2xl lg:text-3xl
                2xl:text-4xl cursor-pointer'
                onClick={handleClearProducts}
              />
              <h2 className='text-black flex pb-5 justify-center w-full text-lg lg:text-2xl xl:text-2xl 2xl:text-4xl'>
                <b>Shopping Cart</b>
              </h2>
              <RxCross2 className='text-black font-bold text-2xl lg:text-3xl
                2xl:text-4xl cursor-pointer'
                onClick={toggleCart}
              />
              <hr />
              ...
              <div className='flex w-full flex-col text-start'>
                ...
                <div className='flex flex-col'>
                  {products.map((product) => (
                    <div key={product.id} className='flex w-full bg-slate-200 h-[85px] sm:h-24 mb-2
                      md:h-24 lg:h-28
                      ...
                      <div className='h-4/5 sm:h-5/6 w-1/4 justify-center items-center flex p-1 bg-slate-300 rounded-2xl'>
                        <img className='h-5/6 w-auto' src={product.thumbnail} alt="product" />
                      </div>
                      <div className='text-start h-full  w-3/4 p-2 box-border'>
                        <div className='w-full flex justify-between'>
                          ...
                          <MdDelete className='text-black text-xs sm:text-sm lg:text-base xl:text-lg 2xl:text-2xl cursor-pointer'
                                    onClick={() => handleRemoveProduct(product.id, product.discountPercentage, product.price,product.quantity)}
                          />
                        </div>
                        ...
                        <p className='text-black sm:text-sm text-xs lg:text-base xl:text-lg 2xl:text-2xl'>
            ...
            <div className='w-full flex text-sm text-black lg:text-lg xl:text-xl 2xl:text-3xl p-3'>
              ...
              <h1 className='w-2/5'><b>${totalBill.toFixed(2)}</b></h1>
            </div>
        ...
        onClick={toggleCart}/>
         ...
         className='border-2 my-5 mx-1 rounded-xl bg-orange-700 text-white text-center cursor-pointer
          ...
          text-xs h-7 w-20
          sm:text-xs sm:h-8 sm:w-20
          md:text-sm md:h-9 md:w-24 
          lg:text-base lg:h-10 lg:w-28
          xl:text-lg xl:h-12 xl:w-32
          2xl:text-xl 2xl:h-14 2xl:w-36'
          onClick={handleClick}/>
